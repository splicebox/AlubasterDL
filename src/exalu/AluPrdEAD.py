...
from .models.Model_CNN_v5_5 import CNet_v5_5
from .models.Model_FCN_v1 import FCN_v1
from .encoding import *
from .load_data import *


class AluPrdEAD(object):
    def __init__(self, datasets, model_name='cnet', num_epochs=140, lr=3e-4, run_mode=0):
        ...
        if model_name == 'cnet':
            self.model = CNet_v5_5(self.batch_size)
        elif model_name == 'fcn':
        ...
        if run_mode == 0 or run_mode == 2:
            ...
            self.dataset_sizes = {'train': len(train_dataset), 'val': len(val_dataset), 'test': len(test_dataset)}
    ...
    def train(self, run_mode=0, work_dir=None, infer_set=None, model_dst_dir=None):
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            print('bce train loss: {:.8f}\tval loss: {:.8f}\ttest loss: {:.8f}'.format(display_loss_bce['train'], display_loss_bce['val'], display_loss_bce['test']))
            print('all train loss: {:.8f}\tval loss: {:.8f}\ttest loss: {:.8f}'.format(display_loss_all['train'], display_loss_all['val'], display_loss_all['test']))
            ...
            if run_mode == 2:
                ...
                perf_results.append(records_dict['val'] | records_dict['test'] | record_loss)
                # print(perf_results)
                continue    
